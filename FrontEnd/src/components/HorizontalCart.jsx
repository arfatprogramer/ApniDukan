...
const HorizontalCart = ({ category, heading }) => {
  ...
  return (
    <div className=' mx-ato px-4 my-6'>
      ...
      <div className="flex  gap-5 overflow-auto scroll-none">
        {
          loading ? (
            new Array(15).fill(null).map((item, index) => {
              return (
                <div key={index} className="w-full min-w-[280px] md:min-w-[220px] max-w-[100px] md:max-w-[120px] h-96 bg-white rounded-sm shadow  overflow-hidden animate-pulse">
              ...
              return (
                <Link to={"/product/"+item?._id} key={index} className="w-full min-w-[280px] md:min-w-[220px] max-w-[100px] md:max-w-[120px] h-96 bg-white rounded-sm shadow  overflow-hidden hover:scale-105 transition-all hover:shadow-xl">
                  ...
                  <div className="p-4 grid gap-2 ">
                    ...
                    <button onClick={(e)=>handelAddToCart(e,item?._id)} className=' text-sm bg-red-500 hover:bg-red-600 text-white px-3 py-2 ph-3 rounded-full'>Add to Cart</button>
                  </div>
